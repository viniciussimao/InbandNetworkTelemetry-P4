...
# Métricas
SWITCH_ENQ_QDEPTH = Gauge('int_switch_enq_qdepth', 'Queue depth at ingress (Queue 1)', ['switch_id'])
HOST_TIMESTAMP = Gauge('int_host_timestamp', 'Host timestamp (Queue 1)')
SWITCH_INGRESS_TS = Gauge('int_switch_ingress_timestamp', 'Ingress timestamp (Queue 1)', ['switch_id'])
SWITCH_EGRESS_TS = Gauge('int_switch_egress_timestamp', 'Egress timestamp (Queue 1)', ['switch_id'])
    ...
    if IP in pkt and pkt[IP].src == "192.168.50.11":
        ...
        if nodeCount in pkt:
            for int_entry in pkt[nodeCount].INT:
                ...
                # Debug (opcional)
                print(f"[Queue 1] Switch {switch_id} - Queue Depth: {int_entry.enq_qdepth}")
...
def main():
    ...
    print("Starting INT collector for Queue 1...")
